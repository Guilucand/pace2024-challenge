...
impl Model<Solving> {
    ...
    /// Get cols
    pub fn get_cols(&self) {
        self.scip.get_cols().unwrap();
    }
...
pub trait ProblemOrSolving {
    ...
    fn del_cons(&mut self, cons: Rc<Constraint>);
    ...
    (for $($t:ty),+) => {
        $(impl ProblemOrSolving for $t {
            ...
            fn del_cons(
                ...
                self.scip
                    .delete_cons(cons)
                    .expect("Failed to delete constraint in state ProblemCreated");
            }
...
impl<T> Model<T> {
    ...
    pub fn hide_output(&mut self) -> &mut Self {
        self.scip
    ...
    /// Shows the output of the optimization model by setting the `display/verblevel` parameter to 1.
    pub fn show_output(&mut self) -> &mut Self {
        self.scip
            .set_int_param("display/verblevel", 5)
            .expect("Failed to set display/verblevel to 5");
        self
    }
