...
impl ScipPtr {
    ...
    pub(crate) fn get_cols(&self) -> Result<(), Retcode> {
        let mut cols: *mut *mut SCIP_COL = null_mut();
        let mut count = 0;

        unsafe {
            scip_call!(ffi::SCIPgetLPColsData(
                self.raw,
                &mut cols as *mut _,
                &mut count as *mut _
            ));

            let is_basic = ffi::SCIPisLPSolBasic(self.raw);

            if count > 0 {
                println!("Cols: {} {:?} basic: {}", count, cols, is_basic);
                for idx in 0..count {
                    let col = *cols.add(idx as usize);
                    let var = ffi::SCIPcolGetVar(col);
                    let name = CStr::from_ptr(ffi::SCIPvarGetName(var)).to_str().unwrap();

                    let reduced = ffi::SCIPgetColRedcost(self.raw, col);

                    println!("Var: {:?} {} red cost: {}", var, name, reduced);
                }
            }
        }
    ...
    pub(crate) fn delete_cons(&mut self, cons: Rc<Constraint>) -> Result<(), Retcode> {
        scip_call! { ffi::SCIPdelCons(self.raw, cons.raw) };
        let mut rawp = cons.raw;
        scip_call! { ffi::SCIPreleaseCons(self.raw, &mut rawp as *mut _) };
        Ok(())
    }
